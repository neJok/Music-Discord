...
export default new Interaction(
    ...
    async (client: MusicCenter, button: ButtonInteraction<'cached'>) => {
        ...
        const status = (((queue.songs[0].info.start || 0) + queue.songs[0].info.length ) - Date.now()) > 10000

        await player.seek(status ? (Date.now() - queue.songs[0].info.start) + 10000 : queue.songs[0].info.length-1000)
        queue.songs[0].info.start = (status ? (queue.songs[0].info.start - 10000) : Date.now()-1000)

        return button.editReply({
            embeds: [ new MusicEmbed().success(`Вы ${status ? 'перемотали на 10 секунд вперед' : 'перемотали в конец трека'}`) ]
        })
    }
)